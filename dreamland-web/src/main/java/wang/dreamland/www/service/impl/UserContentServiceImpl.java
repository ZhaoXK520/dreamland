...
public class UserContentServiceImpl implements UserContentService {
    ...
    public UserContent findById(long id,int audit) {
        UserContent userContent = new UserContent();
        userContent.setId( id );
        userContent.setAudit(audit);
        List<UserContent> list = userContentMapper.findByJoin(userContent);
        if(list!=null && list.size()>0){
            return list.get(0);
        }else {
    ...
    public int updateById(UserContent content) {
    ...
    @Override
    public Integer getUpvoteNum(long uid) {
        return userContentMapper.upvoteNum(uid);
    }
}
